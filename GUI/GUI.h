...
public:
	...
	// Output Functions  ---------------------------
	void PrintMessage(string msg,string msg2,string msg3,string msg4,string msg5) const; // prints a message in the status bar
	void PrintMessage(string msg) const;
	//noha
	void UpdateStatusBar(int CurrentTimeStep, double CastleHealth, ENMY_STATUS Castlestatus, int actv_fighters, int actv_freezer, int actv_healer, int frez_fighters, int frez_freezer, int frez_healer, int kld_fighters, int kld_freezer, int kld_healer);	//prints info on status bar. Info includes timestep and other enemies info as illustrated in project doc
	void UpdateInterface(int CurrentTimeStep, double CastleHealth, ENMY_STATUS Castlestatus, int actv_fighters, int actv_freezer, int actv_healer, int frez_fighters, int frez_freezer, int frez_healer, int kld_fighters, int kld_freezer, int kld_healer);
	void AddToDrawingList( Enemy*);	//Adds a new enemy to the drawing queue
	/*void AddToDrawingListFighter(Fighter* pF);
	void AddToDrawingListHealer(Healer* pH);
	void AddToDrawingListFreezer(Freezer* pFr);*/
